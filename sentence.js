...
function generateUrl(protocol, host, escapedString) {
    return `${protocol}//${host}/program/sentencegenerator-12/#${escapedString}`;
}
        ...
        if (partsKeys[i] == qobj.question.particle) { // if the question is this section of the sentence
            ...
            for (let i in fakes) { // loop through fakes
                ...
                if (qobj.question.type == "particle") { // particle question type
                    ...
                    option.innerHTML = `${fixCapitalization(fake.word)} (${fake.meaning})`;
                }
            ...
            if (qobj.question.type == "particle") {
                ...
                option.innerHTML = `${fixCapitalization(qobj.parts[qobj.question.particle])} (${qobj.meanings[qobj.question.particle]})`; // set the select box to the correct value (e.g. "neko (subject)" or "Ute (object subject)")
            }
